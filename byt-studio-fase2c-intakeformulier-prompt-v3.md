# CLAUDE CODE PROMPT — Fase 2C: Intakeformulier BYT Studio (v3, DEFINITIEVE huisstijl)
...
**Dit vervangt alle eerdere intakeformulier-prompts (v1, v2). Voer eerst `byt-studio-huisstijl-prompt-v3-definitief.md` uit als dat nog niet gebeurd is — dit bouwt verder op die tokens.**
...
**Belangrijk: dit zijn de definitieve tokens uit het officiële Studio BYT Brand Book (`brandbook.css`). Als je project deze al heeft (via de v3-huisstijl-migratie), hergebruik die — maak geen nieuwe/parallelle kleurdefinities aan.**
...
font-family (body/UI): 'Hanken Grotesk', sans-serif;
font-family (labels/mono/eyebrow): 'JetBrains Mono', monospace;
```

Gebruik `--green` (`#22C35D`) voor vlakken/knoppen/actieve stap-indicatoren, en `--forest` (`#0B7A38`) voor kleine groene tekst op een lichte achtergrond (beter contrast) — dat is het patroon dat het brandbook zelf consequent toepast. `--coral` enkel spaarzaam als accent (bv. foutmeldingen, must-have-badge bij features), nooit als hoofdkleur.
...
- **Publieke pagina:** **geen** app-navigatie/sidebar. Losstaande pagina met een rustige header: logo bovenaan (klein), `--carbon` (`#0E1512`) achtergrond in de header, `--mist` (`#F5F7F5`) als achtergrond voor de formuliersecties zelf.

**Logo:** gebruik de assets uit `public/assets/logo/` en `public/assets/mark/` (dezelfde set als gebruikt bij de v3-huisstijl-migratie).
- Publieke pagina-header (donkere achtergrond) → `logo-reversed.svg`
- Bedankscherm/kleine iconen → `mark-white.svg` (op donkere achtergrond) of `mark-black.svg` (op lichte achtergrond)
...
- Stap-indicator: 6 bolletjes/nummers bovenaan, huidige stap gemarkeerd in `--green` (`#22C35D`), afgewerkte stappen in `--forest` (`#0B7A38`).
- Elke stap in een kaart (afgeronde hoeken `14px`, rand in `--cloud` (`#E6EAE7`), witte/`--mist`-achtergrond).
